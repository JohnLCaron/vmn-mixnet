...
// publish/import ballots to/from ByteTree. Switch group as needed
...
fun ByteTreeNode.importMixnetBallots(group: GroupContext) : List<MixnetBallot> {
    ...
    repeat(ntexts) { textidx ->
        ...
        repeat(pads.childs()) { idx ->
            val pad = pads.child[idx].content!!
            val data = datas.child[idx].content!!
            ciphertexts.add(importText(group, pad, data))
        }
...
fun makeNode(name: String, listOflist : List<List<ElementModP>>): ByteTreeNode {
    ...
    listOflist.forEachIndexed { idx, inner : List<ElementModP> ->
        ...
        inner.forEachIndexed { idx, it ->
            val btNode: ByteTreeNode = ElementBt(it).publish() // heres where we add the extra leading 0 byte
            innerNodes.add( btNode)
        }
...
fun importText(group: GroupContext, pad: ByteArray, data: ByteArray): ElGamalCiphertext {
    return ElGamalCiphertext(group.binaryToElementModP(pad)!!, group.binaryToElementModP(data)!!)
}