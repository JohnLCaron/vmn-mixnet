...
// public key y = g^x
data class ModPublicKey(val publicKey : ElementModP, val encoding: Int = 0) {
    ...
    fun publish() : ByteTreeNode {
        val modPGroupBt = (publicKey.group as IntGroupContext).makeModPGroupBt(this.encoding)

        return makeNode("MixnetPublicKey",
            listOf(
                modPGroupBt.publish(),
                makeNode("group", listOf(
                    makeLeaf("g", this.publicKey.group.G_MOD_P.byteArray().normalize(513)),
 ...
 */
fun ByteTreeNode.importModPublicKey(group: GroupContext) : ModPublicKey {
    val mgroup = this.child[0].importModPGroup()
    val keys = this.child[1]

    val g = group.binaryToElementModP(keys.child[0].content!!)!!
    val pk = group.binaryToElementModP(keys.child[1].content!!)!!

    return ModPublicKey(pk, mgroup.encoding)
}